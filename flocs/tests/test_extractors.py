...
def test_select_random_task():
    state_a = empty + StaticContext(randomness=0) + s1 + t1 + t2 + t3
    state_b = empty + StaticContext(randomness=1) + s1 + t1 + t2 + t3
    assert select_random_task(state_a, student_id=1) == 1
    assert select_random_task(state_b, student_id=1) == 2
